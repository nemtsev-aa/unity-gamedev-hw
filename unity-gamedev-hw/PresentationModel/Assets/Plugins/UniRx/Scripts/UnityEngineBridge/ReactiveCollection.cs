...
namespace UniRx
{
    ...
    public class ReactiveCollection<T> : Collection<T>, IReactiveCollection<T>, IDisposable
    {
        ...
        public ReactiveCollection(ReactiveCollection<T> collection)
        {
            if (collection == null) throw new ArgumentNullException("collection");

            collectionMove = collection.collectionMove;
            foreach (var item in collection)
            {
                Add(item);
            }
        ...
        public static void Transfer(ReactiveCollection<T> fromCollection)
        {
