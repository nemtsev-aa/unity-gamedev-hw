...
namespace UniRx
{
    ...
    public interface IMessageReceiver<T> where T : IMessage
    {
        ...
        /// Subscribe typed message.
        /// </summary>
        IObservable<TMessage> Receive<TMessage>() where TMessage : T;
    }
    ...
    public interface IMessageBroker<T> : IMessagePublisher<T>, IMessageReceiver<T> where T : IMessage
    {
    }
    ...
    public class MessageBroker<T> : IMessageBroker<T>, IDisposable where T : IMessage
    {
        bool isDisposed = false;
        readonly Dictionary<Type, object> notifiers = new Dictionary<Type, object>();

        public void Publish<TMessage>(TMessage message) where TMessage : T
        {
            object notifier;
            lock (notifiers)
            {
                ...
                if (!notifiers.TryGetValue(typeof(TMessage), out notifier))
                {
                    return;
                }
            }
        ...
        public IObservable<TMessage> Receive<TMessage>() where TMessage : T
        {
            object notifier;
            lock (notifiers)
            {
                ...
                if (!notifiers.TryGetValue(typeof(TMessage), out notifier))
                {
                    ISubject<TMessage> n = new Subject<TMessage>().Synchronize();
                    notifier = n;
                    notifiers.Add(typeof(TMessage), notifier);
                }
            }

            return ((IObservable<TMessage>) notifier).AsObservable();
        }
