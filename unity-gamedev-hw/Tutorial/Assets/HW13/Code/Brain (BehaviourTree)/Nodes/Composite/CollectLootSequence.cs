...
namespace BehaviorTree.Brain {
    ...
    [AddComponentMenu("")]
    [MBTNode("Main Companents/" + nameof(CollectLootSequence))]
    public class CollectLootSequence : Composite {
        ...
        private bool _collectSucces = false;

        private Node MoveNode => children[0];
        private Node CollectNode => children[1];

        public override void OnEnter() {
            base.OnEnter();

            if (MoveNode == null || CollectNode == null)
        ...
        public override NodeResult Execute() {
            ...
            if (_botBrainData.NearestResourceLoot.CurrentValue == null)
            ...
            if (_collectSucces == false) {

                CollectNode.OnEnter();
                _collectSucces = NodeExecute(CollectNode, out var collectActionResult);

                if (_collectSucces == false)
                    return collectActionResult;
            }

            if (_moveSucces == _collectSucces == true) {
